...
const setForecast = data => {
    ...
    speedEl.innerHTML= ` <div> <img class="windicon" src="./src/img/wind.png"></div> Wind Speed: ${speed} m/s <br /> Wind Direction:   ${windDirection}`;
}
...
//   });
// }
export const getForecastData = async (param) =>{
    try{
        let searchBy = (param && param.coords)?`lat=${param.coords.latitude}&lon=${param.coords.longitude}`:`q=${param}`
        const response = await fetch(`${API_URL}${searchBy}&units=metric&appid=${API_KEY}`);
